...
function setup() {
  ...
  //creating background
  ground=createSprite(740,250,200,200);
...
function draw() {
  ...
  if(gameState==="PLAY"){
    ...
    //player jumping     
    if(keyDown("space")&&player.y>239&&player.isTouching(landGroup)){
         player.velocityY=-20;
         jump.play();
         jump.setVolume(0.05)
      }
    ...
    if(player.y>500||starGroup.isTouching(player)){
      ...
      textSize(20);
      text("You ran " +score+" yards!",200,150);
     }
